...
class Noark5ControlOverviewDialog(ctk.CTkToplevel):
    """In-app worklist over one generated Noark 5 batch control overview."""
    ...
    def _refresh_rows(self) -> None:
        ...
        headers = (
            "Jobb", "Navn", "Status", "Arkivdeler", "Tester", "Mismatch",
            "Vurderingssaker", "Funn", "Rapport",
        )
        ...
        for index, row in enumerate(rows, start=1):
            ...
            report_path = Path(row["report_html"]) if row.get("report_html") else None
            ctk.CTkButton(
                self.rows_frame,
                text="Åpne",
                width=70,
                state="normal" if report_path and report_path.is_file() else "disabled",
                command=lambda p=report_path: self._open_report(p),
                fg_color=theme.BUTTON_BG,
                hover_color=theme.BUTTON_HOVER,
            ).grid(row=index * 2 - 1, column=8, padx=6, pady=(4, 2))
    ...
    def _open_report(self, path: Path | None) -> None:
        if path is None or not path.is_file():
            messagebox.showwarning(
                APP_NAME,
                "Rapportfilen finnes ikke lenger på forventet sted.",
                parent=self,
            )
            return
