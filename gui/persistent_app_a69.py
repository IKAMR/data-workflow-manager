...
from .persistent_app_a68 import WorkflowApp as A68WorkflowApp

class WorkflowApp(A68WorkflowApp):
    """v0.1.6-a9: activate explicit archive-part missing-data explanation."""
    def _open_validation_overview(self) -> None:
        ...
        dialog=Noark5ControlOverviewDialogA29(parent,overview_json=json_path,overview_html=html_path,user_identity=self.current_user_identity())
    ...
    def _open_depot_assessment(self) -> None:
        ...
        job=self.current_job; work_operations=None
        if job is not None:
        ...
        if report_path is None:
            dialog=DepotAssessmentDialogA29(self,user_identity=self.current_user_identity(),work_operations=work_operations)
        else:
            dialog=DirectDepotAssessmentDialogA29(self,report_path=report_path,user_identity=self.current_user_identity())
