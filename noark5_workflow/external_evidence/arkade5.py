...
def _validate_report(report: dict[str, Any]) -> None:
    ...
    invalid = [item for item in tests if not isinstance(item, dict) or not str(item.get("TestId") or "").startswith("N5.")]
    if invalid:
        raise Arkade5ImportError("TestsResults inneholder elementer som ikke gjenkjennes som Noark 5-tester.")


def _infer_source_version(source_path: Path) -> str | None:
    for part in reversed(source_path.parts):
        match = re.search(r"arkade5[_-]?v?(\d+(?:\.\d+)+)", part, flags=re.IGNORECASE)
        if match:
            return match.group(1)
    return None


def _flatten_result_set(result_set: dict[str, Any] | None, path: tuple[str, ...] = ()) -> list[dict[str, Any]]:
    ...
    for raw in result_set.get("Results") or []:
        ...
        flattened.append({
            ...
            "location": {
                ...
                "line_numbers": loc.get("LineNumbers"),
            },
...
def normalize_arkade5_report(report: dict[str, Any], *, source_file: str = "", source_sha256: str = "", source_version: str | None = None) -> dict[str, Any]:
    """Normalize one Arkade 5 JSON test report without interpreting it as a depot decision."""
    ...
    for raw in report.get("TestsResults") or []:
        ...
        try:
            ...
            error_count = 0
        result_types = {str(item.get("result_type") or "").casefold() for item in result_items}
        ...
        tests.append({
            ...
            "test_description": raw.get("TestDescription"),
    ...
    summary = {
        "uuid": source_summary.get("Uuid"),
        "archive_type": source_summary.get("ArchiveType"),
        "archive_creators": source_summary.get("ArchiveCreators"),
        "archival_period": source_summary.get("ArchivalPeriod"),
        "system_name": source_summary.get("SystemName"),
        "system_type": source_summary.get("SystemType"),
        "date_of_testing": source_summary.get("DateOfTesting"),
        "number_of_processed_files": source_summary.get("NumberOfProcessedFiles"),
        "number_of_processed_records": source_summary.get("NumberOfProcessedRecords"),
        "number_of_tests_run": source_summary.get("NumberOfTestsRun"),
        "number_of_errors": source_summary.get("NumberOfErrors"),
        "number_of_warnings": source_summary.get("NumberOfWarnings"),
    }
    return {
...
def build_arkade5_reconciliation(normalized: dict[str, Any], depot_model: dict[str, Any]) -> dict[str, Any]:
    """Compare only mappings explicitly marked equivalent in arkade5_mapping.json."""
    mapping = load_arkade5_mapping()
    tests = {str(item.get("test_id")): item for item in normalized.get("tests") or []}
    ...
    for rule in mapping.get("mappings") or []:
        ...
        arkade_value = _extract_arkade_value(test or {}, str(rule.get("arkade_selector") or "")) if test else None
    ...
    return {
        "format_version": 1,
        "comparison": "arkade5_to_n5wf_depot_report",
...
def infer_work_operations_from_depot_report(report_path: str | Path) -> Path:
    ...
    raise Arkade5ImportError("Kunne ikke finne work_operations fra depotrapportens plassering.")
...
def import_arkade5_report(
    ...
    *,
    ...
    """Import Arkade evidence into work_operations and return its manifest.

    The original report is copied byte-for-byte. The import is stable per source
    SHA-256, so importing the same report again returns/refreshes the same evidence
    directory instead of silently creating duplicates.
    """
    ...
    if iso_match:
        ...
        no_match = re.search(r"(\d{1,2})\.?\s+([A-Za-zæøåÆØÅ]+)\s+(\d{4})", raw_date)
        if no_match:
    ...
    if depot_report_path is not None:
        ...
        try:
            ...
            raise Arkade5ImportError(f"Kunne ikke lese N5WF depotrapporten: {exc}") from exc
        reconciliation = build_arkade5_reconciliation(normalized, depot_model)
        reconciliation_path = reconciliation_dir / "arkade5_n5wf_reconciliation.json"
    ...
    manifest = {
        ...
        "source": {
            ...
            "sha256": digest,
            "preserved_file": str(copied_source.relative_to(Path(work_operations))),
        },
        "normalized_file": str(normalized_path.relative_to(Path(work_operations))),
        "reconciliation_file": (
            str(reconciliation_path.relative_to(Path(work_operations))) if reconciliation_path else None
        ),
...
def list_arkade5_imports(work_operations: str | Path) -> list[dict[str, Any]]:
    ...
    return sorted(imports, key=lambda item: str(item.get("imported_at") or ""), reverse=True)


def load_arkade5_import(work_operations: str | Path, import_id: str) -> dict[str, Any]:
    root = Path(work_operations) / "external_evidence" / "arkade5" / import_id
    manifest = _read_json(root / "manifest.json")
    normalized = _read_json(Path(work_operations) / str(manifest.get("normalized_file")))
    reconciliation = None
    rec_file = manifest.get("reconciliation_file")
    if rec_file:
        reconciliation = _read_json(Path(work_operations) / str(rec_file))
    return {"manifest": manifest, "normalized": normalized, "reconciliation": reconciliation}
