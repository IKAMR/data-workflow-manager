...
MATURITY_LEVELS = {"alpha": 0, "beta": 1, "stable": 2}
MATURITY_LABELS = {"alpha": "Alpha", "beta": "Beta", "stable": "Stabil"}
MATURITY_SHORT_LABELS = {"alpha": "A", "beta": "B", "stable": "S"}
VISIBILITY_LABELS = {0: "Alle (inkl. Alpha)", 1: "Beta og stabile", 2: "Kun stabile"}
...
def load_operation_metadata() -> dict:
    ...
    return {**data, "operations": operations if isinstance(operations, dict) else {},
...
def maturity_name(operation_id: str) -> str:
    maturity = str(_operation_metadata(operation_id).get("maturity", "alpha")).strip().lower()
    return maturity if maturity in MATURITY_LEVELS else "alpha"

def maturity_level(operation_id: str) -> int: return MATURITY_LEVELS[maturity_name(operation_id)]
def maturity_label(operation_id: str) -> str: return MATURITY_LABELS[maturity_name(operation_id)]
def maturity_short_label(operation_id: str) -> str: return MATURITY_SHORT_LABELS[maturity_name(operation_id)]

def is_visible(operation_id: str, minimum_level: int) -> bool:
    try: threshold = int(minimum_level)
    except (TypeError, ValueError): threshold = 2
    return maturity_level(operation_id) >= max(0, min(2, threshold))
...
def visibility_value(label: str) -> int: return VISIBILITY_VALUES.get(str(label), 2)
...
def display_order(operation_id: str) -> int:
    try: return int(_operation_metadata(operation_id).get("display_order", 999))
    except (TypeError, ValueError): return 999

def catalogue_visible(operation_id: str) -> bool:
    return _operation_metadata(operation_id).get("catalog_visible", True) is not False
...
def belongs_to_profile(operation_id: str, profile_id: str) -> bool:
    ...
    if raw and not catalogue_visible(operation_id):
        return False
...
def display_category_names(operations: Iterable, profile_id: str) -> list[str]:
    ...
    for operation in operations:
        ...
        if belongs_to_profile(op_id, profile_id):
